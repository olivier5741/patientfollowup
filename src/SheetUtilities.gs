...
function convertSheetToPdf(spreadsheet, sheet, pdfName, lastColumn = columnToLetter(spreadsheet.getLastColumn())) {
  ...
  const url_ext = 'export?exportFormat=pdf&format=pdf'   //export as pdf
      ...
      + '&printnotes=false'
      + '&range=a1:'+ lastColumn + spreadsheet.getLastRow();       // range to export TODO more than n if more than 14 days
